...
import com.xq.tmall.config.AlipayConfig;
...
public class ForeOrderController extends BaseController {
    ...
    public String goToOrderPayPage(Map<String, Object> map, HttpSession session,
        ...
        return "fore/productPayPage1";
    }

    //转到支付宝支付
    @RequestMapping("order/payment")
    public String payment(HttpServletRequest request, Model model, String productOrder_code) {
        try {
            //获得初始化的AlipayClient
            ...
            ProductOrder order = productOrderService.getByCode(productOrder_code);

            //商户订单号，商户网站订单系统中唯一订单号，必填
            String out_trade_no = new String(request.getParameter("productOrder_code").getBytes("UTF-8"),"UTF-8");
            //付款金额，必填
            String total_amount = new String(request.getParameter("orderTotalPrice").getBytes("UTF-8"),"UTF-8");
            //订单名称，必填
            String subject = "用户为"+order.getProductOrder_user()+"的订单";
            //商品描述，可空
            String body = "";
            ...
            //修改订单状态
            productOrderService.updateById(out_trade_no,1);
            return "fore/payment";
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    //完成支付
    @RequestMapping("order/completePay")
